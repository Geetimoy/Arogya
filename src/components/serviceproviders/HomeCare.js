...
function HomeCare() {
  ...
  const [providerId, setProviderId] = useState('10');
  const [providerList, setProviderList] = useState([]);
  const [totalCount, setTotalCount] = useState(0);
  const [loadMore, setLoadMore] = useState(false);
  const [offset, setOffset] = useState(0);

  const searchHomeCare = (e) => {
    setProviderList([]); // Reset child list when searching
    setLoadMore(false); // Reset load more state
    setTotalCount(0); // Reset total count
    setTimeout(()=>{
      listHomeCare(searchRef.current.value, 0); // Call listChild with search key
    }, 500)
  }

  const listHomeCare = async (searchKey, customOffset) => {
  ...
  const loadMoreHomeCare = () => {
    listHomeCare(searchRef.current.value, offset); // Load more data
  }
    ...
    <div className="app-body service-provider">
      <div className='search-patient mt-3 mb-3'>
        <div className='input-group'>
          ...
          <span className="input-group-text" onClick={searchHomeCare}><FontAwesomeIcon icon={faSearch} /></span>
        </div>
      </div>
      <div className='row'>
        ...
        {loadMore && <div className='col-12 text-center'>
          <Link to="#" className='btn btn-primary primary-bg-color border-0' onClick={loadMoreHomeCare}>Load More</Link> 
        </div>}
      </div>
    </div>
    <Appfooter></Appfooter>
