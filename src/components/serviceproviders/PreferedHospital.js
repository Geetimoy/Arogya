...
function PreferedHospital() {
  ...
  const [providerId, setProviderId] = useState('9');
  const [providerList, setProviderList] = useState([]);
  const [totalCount, setTotalCount] = useState(0);
  const [loadMore, setLoadMore] = useState(false);
  const [offset, setOffset] = useState(0);

  const searchPreferedHospital = (e) => {
    setProviderList([]); // Reset child list when searching
    setLoadMore(false); // Reset load more state
    setTotalCount(0); // Reset total count
    setTimeout(()=>{
      listPreferedHospital(searchRef.current.value, 0); // Call listChild with search key
    }, 500)
  }

  const listPreferedHospital = async (searchKey, customOffset) => {
  ...
  useEffect(() => {
    if(systemContext.systemDetails.system_id){
      listPreferedHospital("", 0);
    }
    // eslint-disable-next-line
  }, [systemContext.systemDetails.system_id]);

  const loadMorePreferedHospital = () => {
    listPreferedHospital(searchRef.current.value, offset); // Load more data
  }
    ...
    <div className='app-top inner-app-top services-app-top'>
      <div className='app-top-box d-flex align-items-center justify-content-between'>
        <div className='app-top-left d-flex align-items-center'>
          ...
          <h5 className='mx-2 mb-0'>Prefered Hospital</h5>
        </div>
    ...
    <div className="app-body service-provider">
      <div className='search-patient mt-3 mb-3'>
        <div className='input-group'>
          <input type="text" className='form-control' placeholder='Search Hospital' id="searchHospital" name="searchHospital"  />
          <span className="input-group-text" onClick={searchPreferedHospital}><FontAwesomeIcon icon={faSearch} /></span>
        </div>
      </div>
      <div className='row'>
        ...
        {loadMore && <div className='col-12 text-center'>
          <Link to="#" className='btn btn-primary primary-bg-color border-0' onClick={loadMorePreferedHospital}>Load More</Link> 
        </div>}
...
export default PreferedHospital;