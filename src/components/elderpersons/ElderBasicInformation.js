...
function ElderBasicInformation(){
  ...
  const handleFormSubmit = async (e) => {
        ...
        if(errorCounter === 0){
          ...
          const response = await fetch(`${API_URL}/addUpdateElderBasicInformation`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  const getUserDetails = async () => {
    ...
    if(result1.data.length > 0){
      ...
      if(result1.data.length > 0){
        ...
        formData['emergency_contact_name']       = {required:formData['emergency_contact_name'].required, value:userDetails.emergency_contact_name || "", errorClass:"", errorMessage:""};
