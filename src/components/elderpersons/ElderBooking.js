...
function ElderBooking(){
  ...
  const [confirmScheduleId, setConfirmScheduleId]             = useState('');
    ...
    const fetchElderDetails = async () => {
      ...
      setElderDetails(result1.data[0]);
    ...
    useEffect(() => {
      if(systemContext.systemDetails.system_id){
        listSchedule("");
        fetchElderDetails(editAccountKey);
      }
      ...
      <div className="row">
              ...
              {scheduleList.map((schedule) => {
                ...
                return <div className="button-box mb-3" key={schedule.doctor_avail_schedule_id}> 
                  ...
                  <div className="mb-3 mt-3 text-center">
                    <button onClick={() => { modalBookingConfirmationShow(schedule.doctor_avail_schedule_id, schedule.account_key); }} className="btn primary-bg-color text-light">Book For Elder</button>
                  </div>
                </div>
        ...
        <Modal show={showBookingConfirmationModal} onHide={modalBookingConfirmationClose}>
          ...
          <Modal.Body> 
            ...
            <p>
              <strong>Elder Name: </strong>{elderDetails.elder_name}<br/>
          ...
          <Modal.Footer className='justify-content-center'> 
            <Button variant="primary" className='btn primary-bg-color text-light border-0 min-width-100 bg-success' onClick={() => confirmBooking(confirmScheduleId, confirmDoctorAccountKey.toLowerCase(), elderDetails.account_key.toLowerCase(), selectedScheduleDate, selectedScheduleTimeFrom, selectedScheduleTimeTo)}>Confirm</Button>  
