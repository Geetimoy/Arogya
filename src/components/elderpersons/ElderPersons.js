...
function ElderPersons(){
  ...
  const [reviewElderName, setReviewElderName] = useState('');

  const [showModal2, setShowModal2] = useState(false); 
  const modalClose2  = () => setShowModal2(false); 
  const [reviewComment, setReviewComment] = useState('');
  const [reviewRating, setReviewRating]   = useState('0'); 
  const modalShow2   = async(elderAccountKey, elderName) => {
     setReviewElderKey(elderAccountKey);
      setReviewElderName(elderName);

     let jsonData = {};
    jsonData['system_id']                 = systemContext.systemDetails.system_id;
    jsonData["account_key"]               = elderAccountKey;
  ...
  const [reviewElderKey, setReviewElderKey] = useState('');
    const handleSaveRating = async () => {
      ...
      try {
        ...
        let jsonData = {
          ...
          // RECEIVER (Elder/patient)
          'receiver_acc_type': "3", // Elder type (confirm once)
          'receiver_acc_key': reviewElderKey,
      ...
      <div className="app-body patient-profiles profile-listing elder-persons">
        ...
        {elderList.map((elder, index) => (
                ...
                <div className={`my-element2 three-dot ${openMenuId === elder.account_id ? 'active' : ''}`} onClick={() => handleMenuClick(elder.account_id)}>
                  ...
                  {openMenuId === elder.account_id && <div className='drop-menu'>
                      <ul>
                        ...
                        <li>
                          {
                            <Link to={`/elderpersons/growth-tracker/${elder.account_key}/from-listing`}>Health Data</Link>
                          }
                        </li>
                        <li>
                      ...
                      (decryptedLoginDetails.account_type !== '5') &&
                        ...
                        <li><Link onClick={() => { modalShow2(elder.account_key, elder.elder_name); }} to="#">Write/View Review </Link></li>
                      </ul>
                    </div>
                  }
                <Link to="#">
          ...
          {loadMore && <div className='col-12 text-center'>
            <Link to="#" className='btn btn-primary primary-bg-color border-0' onClick={loadMoreElder}>Load More</Link> 
          </div>}
        </div>   
        ...
        <Modal show={showModal2} onHide={modalClose2}>
          ...
          <Modal.Body className='feedback-form'>
            ...
            <p className='mb-0'>Name : {reviewElderName}</p>
