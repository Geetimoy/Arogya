...
function JananiBooking(){
  ...
  const [jananiDetails, setJananiDetails]   = useState({});
  ...
  const fetchJananiDetails = async () => {
    ...
    const response1 = await fetch(`${API_URL}/jananiBasicInformationList`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    setJananiDetails(result1.data[0]);
  ...
  useEffect(() => {
    if(systemContext.systemDetails.system_id){
      listSchedule("");
      fetchJananiDetails(editAccountKey);
    }
  ...
  const confirmBooking = async (scheduleId, doctorAccountKey, editAccountKey, selectedScheduleDate, selectedScheduleTimeFrom, selectedScheduleTimeTo) => {
      ...
      <div className="row">
              ...
              {scheduleList.map((schedule) => {
                ...
                return <div className="button-box mb-3" key={schedule.doctor_avail_schedule_id}> 
                  ...
                  <div className="mb-3 mt-3 text-center">
                    <button onClick={() => { modalBookingConfirmationShow(schedule.doctor_avail_schedule_id, schedule.account_key); }} className="btn primary-bg-color text-light">Book For Janani</button>
                  </div>
                </div>
        ...
        <Modal show={showBookingConfirmationModal} onHide={modalBookingConfirmationClose}>
          ...
          <Modal.Body> 
            ...
            <p>
              <strong>Janani Name: </strong>{jananiDetails.janani_name}<br/>
          ...
          <Modal.Footer className='justify-content-center'> 
            <Button variant="primary" className='btn primary-bg-color text-light border-0 min-width-100 bg-success' onClick={() => confirmBooking(confirmScheduleId, confirmDoctorAccountKey.toLowerCase(), jananiDetails.account_key.toLowerCase(), selectedScheduleDate, selectedScheduleTimeFrom, selectedScheduleTimeTo)}>Confirm</Button>  
